...
from gestures.finger_state import count_extended_fingers, get_finger_states
...
def draw_debug_info(frame, hand, fingers, gesture, stable):
    ...
    # Title
    cv2.putText(frame, "GESTURE DEBUG", (20, y), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
    y += spacing
    ...
    # Gesture info
    cv2.putText(frame, f"Raw: {gesture}", (20, y), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 2)
    y += spacing

    cv2.putText(
        frame,
        f"Stable: {stable or 'WAITING...'}",
        (20, y),
        cv2.FONT_HERSHEY_SIMPLEX,
        0.6,
        (0, 255, 0),
    ...
    if hand:
        ...
        cv2.putText(
            frame,
            f"Fingers Extended: {fingers_count}/5",
            (20, y),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.5,
            (255, 255, 255),
